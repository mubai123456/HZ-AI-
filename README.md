...
- 2026-03-27 15:58：已完成单应用工作台结果区、输入区与提交链路的第一轮体验收口。
- 左侧输入区已删掉大段解释文案，参考图卡片现在支持直接 `查看 / 删除`，更适合快速试错。
- 结果区已改成“主结果优先”：任务会显示模板名/提示词来源，多图支持主舞台左右切换，结果图支持 `左右镜像 / 上下镜像` 预览并下载镜像后的当前图。
- 提交接口现在会优先返回首帧任务快照，再在响应后继续排队重算、飞书同步和派发，提交体感速度已明显加快。
